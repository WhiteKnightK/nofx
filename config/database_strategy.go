...
func (d *Database) GetTraderStrategyStatusByStrategyID(traderID, strategyID string) (*TraderStrategyStatus, error) {
	query := `SELECT id, trader_id, strategy_id, symbol, status, entry_price, quantity, realized_pnl, updated_at 
	...
	err := row.Scan(&s.ID, &s.TraderID, &s.StrategyID, &s.Symbol, &s.Status, &s.EntryPrice, &s.Quantity, &s.RealizedPnL, &s.UpdatedAt)
	if err != nil {
		return nil, err
	}
	return &s, nil
